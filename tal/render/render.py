...
def render_nlos_scene(config_filename, args):
    ...
    assert os.path.exists(config_filename), \
        f'{config_filename} does not exist'

    path, name = os.path.split(config_filename)
    candidate_filename = os.path.join(path, name, f'{name}.yaml')
    if os.path.isdir(config_filename) and os.path.isfile(candidate_filename):
        config_filename = candidate_filename

    assert os.path.isfile(config_filename), \
        f'{config_filename} is not a TAL config file'

    config_dir, _ = os.path.split(config_filename)

    try:
        scene_config = yaml.safe_load(
            open(config_filename, 'r')) or dict()
    ...
    laser_grid_normals = expand(np.array([0, 0, 1]), laser_width, laser_height)
